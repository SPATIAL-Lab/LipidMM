...
#-6.7 YD
#-6.4 Holocene 2.1 Ka
RA.LGM <- c(195.9, 430.3, 465)/(195.9 + 430.3 + 465)

#d2H, use ice volume correcte values
d2H.LGM <- c(-153.1, -148.3, -161.2)

#analytical precision
d2H.sd.LGM <- c(1.2, 0.6, 1)

#d13C
d13C.LGM <- c(-26.9, -29.8, -27.8)

#analytical precision
d13C.sd.LGM <- c(0.1, 0.1, 0.1)

###correction for atm CO2 d13C, should set it as a parameter?
Afr.d13C.mu.cor <- Afr.d13C.mu + 8.3 - 6.5 #LGM d13C correction
...
dat = list(d13C.mu.est = Afr.d13C.mu.cor, d13C.omega.est = Afr.d13C.vcov,
           ...
           d2H.mix = d2H.LGM, d2H.mea.sd = d2H.sd.LGM)
...
#save(Wang.LGM.mix, file = "out/Wang_LGM_results.RData")
